...
TabMap = {
  ...
  ["Metalis"] = "Metalis",
  ["Dreamtime"] = "Dreamtime",
  ["Outpost Omega"] = "Medical Outpost Omega",
  ["Challax"] = "Challax",
  ["Dayni Moon"] = "Dayni Moon",
  ["Inside Clank"] = "Inside Clank",
  ["Quodrona"] = "Quodrona"
}
...
-- use bulk update to pause logic updates until we are done resetting all items/locations
    ...
    if slot_data['all_missions'] == true then
        local obj = Tracker:FindObjectForCode("Missions")
        if obj then
            obj.CurrentStage = 1
        end
    end
    if slot_data['all_cutscenes'] == true then
        local obj = Tracker:FindObjectForCode("Cutscenes")
        if obj then
            obj.CurrentStage = 1
        end
    end
    ...
    if slot_data['progressive_mods'] then
        local obj = Tracker:FindObjectForCode("ProgMods")
        local stage = slot_data['progressive_mods']
        if obj then
            obj.CurrentStage = stage
        end
    end
    if slot_data['progressive_armour'] then
        local obj = Tracker:FindObjectForCode("ProgArmor")
        local stage = slot_data['progressive_armour']
        if obj then
            obj.CurrentStage = stage
        end
    end
    if slot_data['clank_challenges'] then
        local obj = Tracker:FindObjectForCode("ClankChall")
        local stage = slot_data['clank_challenges']
        if obj then
            obj.CurrentStage = stage
        end
    end
    if slot_data['skyboard_challenges'] then
        local obj = Tracker:FindObjectForCode("SkyChall")
        local stage = slot_data['skyboard_challenges']
        if obj then
            obj.CurrentStage = stage
        end
    end
    if slot_data['armour_set_checks'] == true then
        local obj = Tracker:FindObjectForCode("ArmorSets")
        if obj then
            obj.CurrentStage = 1
        end
    end
    ...
    if slot_data['enable_clank_challenge_skill_points'] == true then
        local obj = Tracker:FindObjectForCode("ClankSkills")
        if obj then
            obj.CurrentStage = 1
        end
    end
    if slot_data['enable_skyboard_challenge_skill_points'] == true then
        local obj = Tracker:FindObjectForCode("SkySkills")
        if obj then
            obj.CurrentStage = 1
        end
    end
	...
	if PopVersion >= "0.32.0" then
	    ...
	    table.insert(data_strorage_keys, "rsm_current_planet_"..Archipelago.PlayerNumber.."_"..Archipelago.TeamNumber)
...
function onDataStorageUpdate(key, value, oldValue)
    ...
    if Tracker:FindObjectForCode("AutoTab").Active then
    	if key == "rsm_current_planet_"..Archipelago.PlayerNumber.."_"..Archipelago.TeamNumber and value ~= nil then
